...
class Galil //An instance of the Galil class (Galil object) represents a CONNECTION to a controller (not necessarily the controller itself).  Multiple Galil objects can connect to a single Ethernet controller.
{
GalilPrivate *const d;
public:
   ...
   static std::vector<std::string> addresses();                             //returns list of available addresses to connect to (e.g. "1.2.3.4").  Each item in the list may be fed to the constructor Galil()
                                   ...
                                   Galil(const std::string& address = "");  //constructor opens connection with controller e.g. 192.168.1.2, COM1, /dev/ttyS0, GALILPCI1 /dev/galilpci0 (default constructor with no arguments will bring up a dialog)
