    ...
    : QWidget(parent)
{
    ...
    connect(timer,SIGNAL(timeout()),this,SLOT(curveDataChange()));
//    timer->start(1);
...
void Plotter::drawCurves(QPainter *painter)
{
    ...
    while (i.hasNext()) {
        ...
        for (int j = 0; j < data.count(); ++j) {
            ...
            if(j>0)
                painter->drawLine(QPointF(xlast, ylast),QPointF(x, y));
...
void Plotter::curveDataChange()
{
    ...
    QFile file("C:/Qt/qtcreator-2.5.0/00MyQtCode/MyPracticeCode/MyCom/in1.txt");
    if (file.open(QIODevice::ReadOnly))
    {
        QTextStream in(&file);
        for(int ii=0; ii<counttest; ii++)
        {
            if(!in.atEnd())
            {
